!function (e, t) {
    ...
    const r = new Error("unknown message type"),
        ...
        s = (new Error("called FFmpeg.terminate()"), new Error("failed to import ffmpeg-core.js"));
    let o;
    return self.onmessage = async ({data: {id: n, type: E, data: c}}) => {
        const i = [];
        let p;
        try {
                ...
                case t.LOAD:
                    p = await (async ({coreURL: r, wasmURL: a, workerURL: n}) => {
                        ...
                        try {
                            ...
                            if (r || (r = e.replace("/umd/", "/esm/")), self.createFFmpegCore = (await import(r)).default, !self.createFFmpegCore) throw s
                        }
                        const c = r, i = a || r.replace(/.js$/g, ".wasm"), p = n || r.replace(/.js$/g, ".worker.js");
                        return o = await self.createFFmpegCore({
                            mainScriptUrlOrBlob: `${c}#${btoa(JSON.stringify({
                                wasmURL: i,
                                workerURL: p
                            }))}`
                            ...
                            type: t.LOG,
                            data: e
                        }))), o.setProgress((e => self.postMessage({type: t.PROGRESS, data: e}))), E
                    })(c);
                    break;
                case t.EXEC:
